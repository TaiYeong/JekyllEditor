...
load_dotenv()
PAGE_INFO = {
                "markdown_test_page"    : os.environ['MD_RENDER_WEB'],
                "admin_page"            : os.environ['ADMIN_PAGE'],
                "local_host_page"       : os.environ['LOCAL_HOST_PAGE']
             }
...
class CommandToolkit():
    ...
    def get_repo() -> repo.base.Repo:
        return Repo(os.environ['ROOT_GIT'])
...
class SignalSetter():
    ...
    def do_publish(self) -> None:
        ...
        if _is_clear == True:
            CommandToolkit.show_msg_box(f"Complete publish !\nURL : https://taiyeong.github.io/")
        else:
